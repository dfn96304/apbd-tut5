...
public class EmpDeptSalgradeTests
{
    ...
    public void ShouldReturnEmployeesFromChicago()
    {
        ...
        var result =
            from e in emps
            where (from d in depts
                where d.Loc == "CHICAGO"
                select d.DeptNo).Contains(e.DeptNo)
            select e;
    ...
    public void ShouldMatchEmployeeToSalaryGrade()
    {
        ...
        // Join does not support conditions other than "equals"
        /*var result = from e in emps
                    join s in grades on e.Sal equals s.Losal
                    select new {e.EName, s.Grade};*/
        
        var result = from e in emps
                from s in grades
                where e.Sal >= s.Losal && e.Sal <= s.Hisal
                select new {e.EName, Grade = s.Grade};
    ...
    public void ShouldReturnEmployeesEarningMoreThanDeptAverage()
    {
        ...
        var result = from e in emps
                    where e.Sal > (from e1 in emps where e1.DeptNo == e.DeptNo select e1.Sal).Average()
                    select e.EName;
